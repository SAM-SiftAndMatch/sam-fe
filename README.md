...
Dự án tự động kiểm tra định dạng tên nhánh khi bạn chạy lệnh commit:
- **Không được commit trực tiếp vào nhánh chính**: Nghiêm cấm commit trực tiếp lên các nhánh bảo vệ là `main` hoặc `master`.
- **Cấu trúc đặt tên nhánh**: Nhánh mới bắt buộc phải đặt tên theo định dạng `<type>/<description>`.
  - *Các type hợp lệ:* `feat`, `fix`, `chore`, `docs`, `style`, `refactor`, `test`, `hotfix`, `release`.
  - *Ví dụ:* `feat/add-user-auth`, `fix/login-bug`.

### 3. Tự động kiểm tra chất lượng code & bảo vệ Repo (Husky Git Hooks)
Khi bạn chạy lệnh `git commit`, Husky và các công cụ phát triển sẽ tự động thực hiện chuỗi kiểm tra sau:
1. **Kiểm tra nhánh**: Xác thực tên nhánh hợp lệ và ngăn commit trực tiếp vào `main`/`master`.
2. **Dung lượng file**: Chặn commit các file có dung lượng vượt quá **5MB** (khuyến khích dùng Git LFS đối với file lớn).
3. **Biome check & format**: Quét lỗi tĩnh, định dạng lại code và tối ưu imports đối với các file đang staged (`biome check --staged --write`).
4. **TypeScript check**: Chạy `tsc --noEmit` để đảm bảo dự án không có lỗi kiểu dữ liệu trước khi commit.
5. **Kiểm tra Deep Import**: Quét lỗi vi phạm quy tắc đóng gói feature (Deep Import).
6. **Commitlint**: Xác thực tin nhắn commit đúng chuẩn Conventional Commits.
...
### 4. Đồng bộ hóa thư viện & môi trường
- Sử dụng **pnpm** thay vì npm hay yarn: Khi tải code mới từ Git về, hãy chạy lệnh dưới đây để đồng bộ phiên bản thư viện chính xác từ file `pnpm-lock.yaml`:
  ...
  ```
- Khuyến nghị Node.js phiên bản **`>= 20.0.0`** và pnpm **`>= 11.0.0`**.

---

## 🛠️ Công cụ phát triển trợ giúp (Scripts)

Dự án cung cấp một số công cụ viết bằng Bash nằm trong thư mục `scripts/` giúp tự động hóa quy trình phát triển:

1. **Khởi tạo nhanh một Feature (`scripts/create-feature.sh`)**:
   - Sử dụng lệnh sau để tạo nhanh cấu trúc thư mục feature chuẩn (gồm `api/`, `components/`, `hooks/`, `pages/`, `types/`, tệp `index.ts` và Page component trống):
     ```bash
     ./scripts/create-feature.sh <tên-feature-viết-thường>
     ```
   - *Ví dụ:* `./scripts/create-feature.sh users`

2. **Kiểm tra thủ công Deep Import (`scripts/check-features-imports.sh`)**:
   - Quét toàn bộ dự án để tìm và hiển thị các điểm vi phạm import quá sâu giữa các feature:
     ```bash
     ./scripts/check-features-imports.sh
     ```
