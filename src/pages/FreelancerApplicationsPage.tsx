...
const FreelancerApplicationsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved.length === 0) {
      ...
      // Sort newest first
      setApplications(saved.sort((a: Application, b: Application) => new Date(b.appliedAt).getTime() - new Date(a.appliedAt).getTime()));
    }
  }, []);
  ...
  const getStatusBadge = (status: ApplicationStatus) => {
    switch (status) {
      case 'approved':
        return <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-bold">Đã duyệt</span>;
      case 'rejected':
        return <span className="bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-bold">Từ chối</span>;
      case 'draft':
        return <span className="bg-gray-100 text-gray-700 px-3 py-1 rounded-full text-xs font-bold">Bản nháp</span>;
      case 'pending':
      default:
        return <span className="bg-yellow-100 text-yellow-700 px-3 py-1 rounded-full text-xs font-bold">Đang chờ</span>;
    }
  };
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 md:px-10 py-10">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-gray-500">Quản lý và theo dõi trạng thái các công việc bạn đã gửi hồ sơ</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredApps.length === 0 ? (
            ...
            filteredApps.map((app) => (
              <div key={app.id} className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm hover:shadow-md transition-shadow group">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <h2 
                      onClick={() => navigate(paths.PATH_JOB_DETAIL.replace(':id', app.jobId.toString()))}
                  ...
                  <div className="flex items-center gap-3 w-full md:w-auto">
                    {app.status === 'draft' ? (
                      <button
                        onClick={() => navigate(paths.PATH_JOB_APPLY.replace(':id', app.jobId.toString()))}
