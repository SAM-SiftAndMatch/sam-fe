...
const PROPOSALS = [
  {
    ...
    deliveryTime: '5 ngày',
    coverLetter: 'Chào bạn, tôi là Fullstack Developer với hơn 4 năm kinh nghiệm. Tôi đã từng tích hợp thành công VNPay, Momo và ZaloPay cho 3 dự án Next.js (App Router). Tôi có thể đảm bảo code clean, xử lý webhook bảo mật và bàn giao đúng hạn.',
    isTopRated: true,
  },
  {
    id: 'p2',
    ...
    deliveryTime: '7 ngày',
    coverLetter: 'Xin chào, tôi chuyên làm backend Node.js và có sẵn bộ thư viện xử lý chữ ký số (checksum) của VNPay. Việc tích hợp lên Next.js frontend sẽ rất nhanh chóng. Rất mong được hợp tác với bạn.',
    isTopRated: false,
  }
];
        ...
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-2/3 flex flex-col gap-6">
            <div className="bg-white rounded-[24px] p-8 border border-gray-100 shadow-[0_2px_15px_rgb(0,0,0,0.03)]">
              ...
              <div className="flex items-center gap-4 text-sm font-semibold text-gray-500 mb-6">
                <span>Mã DA: <span className="text-gray-900">{project.code}</span></span>
                <span>•</span>
                <span>Ngân sách: <span className="text-[#1D4ED8]">{project.budget}</span></span>
              </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                <h2 className="text-xl font-bold text-gray-900">Đề xuất từ Freelancer ({PROPOSALS.length})</h2>
              </div>
              
              <div className="flex flex-col gap-4">
                {PROPOSALS.map((proposal) => (
                  <div key={proposal.id} className="bg-white rounded-[24px] p-6 border border-gray-100 shadow-[0_2px_15px_rgb(0,0,0,0.03)] flex flex-col md:flex-row gap-6">
                    {/* Info Freelancer */}
                    <div className="flex flex-col items-center text-center md:w-1/4">
                      <div className="relative mb-3">
                        <img src={proposal.avatar} alt="Avatar" className="w-16 h-16 rounded-full object-cover border-2 border-white shadow-sm" />
                        {proposal.isTopRated && (
                      ...
                      <h4 className="font-bold text-gray-900 text-sm mb-1">{proposal.freelancerName}</h4>
                      <div className="flex items-center gap-1 text-xs font-bold text-yellow-500">
                        <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                        {proposal.rating} <span className="text-gray-400">({proposal.reviews})</span>
                      </div>
                    </div>
                    ...
                    <div className="md:w-3/4 flex flex-col">
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex flex-col">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Giá đề xuất</span>
                          <span className="text-lg font-bold text-[#1D4ED8]">{proposal.proposedPrice}</span>
                        </div>
                        <div className="flex flex-col text-right">
                          <span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Thời gian giao</span>
                          <span className="text-sm font-bold text-gray-900">{proposal.deliveryTime}</span>
                        </div>
                      </div>
          ...
          <div className="lg:w-1/3 flex flex-col gap-6">
            ...
            <div className="bg-[#EEF2FF] rounded-[24px] p-6 border border-[#DCE4FF] text-center flex flex-col items-center">
              ...
              <p className="text-sm text-[#1D4ED8]/80 mb-4">Đội ngũ hỗ trợ của SAM luôn sẵn sàng giải đáp thắc mắc.</p>
